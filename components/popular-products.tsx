...
const placeholderProducts = [
  ...
  { id: 5, name: 'RTX 3080 Graphics Card', price: 699.99, badge: 'High Performance' },
  { id: 6, name: 'SteelSeries Arctis Pro', price: 179.99, badge: 'Premium Audio' },
  { id: 7, name: 'Logitech G Pro X Superlight', price: 149.99, badge: 'Ultra Lightweight' },
...
export default function PopularProducts() {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const queries = ['PlayStation 5', 'Xbox Series X', 'Nintendo Switch', 'Gaming Headset', 'Graphics Card', 'Gaming Mouse', 'Gaming Monitor', 'Gaming Keyboard'];
        ...
        const updatedProducts = placeholderProducts.map((product, index) => ({
          ...product,
          image: responses[index]?.data?.results[0]?.urls?.regular || '/placeholder.jpg',
        }));
  ...
  const nextProducts = () => {
    setCurrentIndex((prevIndex) => (prevIndex + productsPerPage) % products.length);
  };

  const prevProducts = () => {
    setCurrentIndex((prevIndex) => (prevIndex - productsPerPage + products.length) % products.length);
  };
        ...
        <div className="relative">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {visibleProducts.map((product) => (
              <Card key={product.id} className="bg-[#1C1917] border-green-800 overflow-hidden">
                <div className="relative">
                  <Image
                    src={product.image}
                    alt={product.name}
                  ...
                  />
                  <Badge className="absolute top-2 right-2 bg-[#22C55E] text-[#121212]">{product.badge}</Badge>
                </div>
                <CardContent className="p-4">
                  <h3 className="text-lg font-semibold mb-2 text-white">{product.name}</h3>
                  <p className="text-[#22C55E] font-bold text-xl">${product.price.toFixed(2)}</p>
                </CardContent>
